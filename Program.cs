            ...
            while (!cts.IsCancellationRequested && bot is not null)
            {
                ...
                foreach (var update in updates)
                {
                    ...
                    try
                    {
                        ...
                        else Log("Получено обновление, в котором нет сообщения: {0}", update.Type.ToString() ?? "NULL");
                    }
                    catch (Exception ex)
                    {
